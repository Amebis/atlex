...
//
static DWORD _AddEAPMessage(_Inout_ AL::TLS::CSessionData *pSessionData, _In_bytecount_(dwEAPAttributeSize) LPCBYTE pbEAPAttribute, _In_ DWORD dwEAPAttributeSize, _Inout_ AL::EAP::CPacket &pktSend, _Out_ EapPeerMethodOutput* pEapPeerMethodOutput);
...
DWORD AuthHandleInnerEAPHOSTAuthentication(_Inout_ AL::TLS::CSessionData *pSessionData, _Out_ AL::EAP::CPacket &pktSend, _Out_ EapPeerMethodOutput* pEapPeerMethodOutput)
{
    ...
    switch (pSessionData->m_Inner.m_EapState) {
        case AL::EAP::INNERSTATE_Start:
            ...
            //
            ccInnerEapIdentity = (DWORD)wcslen(pSessionData->m_user.pwcIdentity);

            !!if ((pcInnerEapIdentity = (PCHAR)malloc(ccInnerEapIdentity + 1)))
            {
                WideCharToMultiByte(CP_UTF8, 0, pSessionData->m_user.pwcIdentity, -1, pcInnerEapIdentity, ccInnerEapIdentity + 1, NULL, NULL);
            } else {
                AL_TRACE_INFO(_T("not enough memory"));
...
DWORD AuthHandleInnerEAPAuthentication(_Inout_ AL::TLS::CSessionData *pSessionData, _Out_ AL::EAP::CPacket &pktSend, _Out_ EapPeerMethodOutput* pEapPeerMethodOutput)
{
    ...
    switch (pSessionData->m_Inner.m_EapState) {
        case AL::EAP::INNERSTATE_Start: {
            ...
            if ((dwReturnCode = AL::TLS::AuthMakeEAPResponseAttribute(0x01, 0x00, pSessionData->m_bCurrentMethodVersion, (LPCBYTE)(LPCSTR)sIdentity, (DWORD)strlen(sIdentity), &pbEAPAttribute, &dwEAPAttributeSize)) == NO_ERROR) {
                LPBYTE pbEAPAVP;
                DWORD dwEAPAVPSize;
                if ((dwReturnCode = AL::TLS::AuthMakeDiameterAttribute(0x4F, pbEAPAttribute, dwEAPAttributeSize, &pbEAPAVP, &dwEAPAVPSize)) == NO_ERROR) {
                    dwReturnCode = AL::TLS::AddEAPMessage(pSessionData, pbEAPAVP, dwEAPAVPSize, pktSend, pEapPeerMethodOutput);
                    AL::Heap::Free((LPVOID*)&pbEAPAVP);
                }
            ...
            if (pSessionData->m_TLSSession.m_pktInnerEAPMsg != NULL) {
                ...
                switch (pSessionData->m_TLSSession.m_pktInnerEAPMsg->Code) {
                    ...
                    case EAPCODE_Request:
                        ...
                        if (pSessionData->m_TLSSession.m_pktInnerEAPMsg->Data[0] != pSessionData->m_Inner.m_eapcfg.m_dwType) {
                            //
                            ...
                            if ((dwReturnCode = AL::TLS::AuthMakeEAPResponseAttribute(0x03, pSessionData->m_TLSSession.m_pktInnerEAPMsg->Id, pSessionData->m_bCurrentMethodVersion, (LPBYTE) &(pSessionData->m_Inner.m_eapcfg.m_dwType), 1, &pbEAPAttribute, &dwEAPAttributeSize)) == NO_ERROR) {
                                dwReturnCode = _AddEAPMessage(pSessionData, pbEAPAttribute, dwEAPAttributeSize, pktSend, pEapPeerMethodOutput);
                                AL::Heap::Free((LPVOID*)&pbEAPAttribute);
                            }
                        }
                        break;
                        ...
                        if (InnerEapOutput.fInvokeInteractiveUI) {
                            ...
                            switch (InnerEapOutput.Action) {
                                ...
                                case EAPACTION_SendWithTimeoutInteractive:
                                    ...
                                    dwReturnCode = _AddEAPMessage(pSessionData, (LPCBYTE)pInnerEapSendPacket, AL::Convert::N2H16(pInnerEapSendPacket->Length), pktSend, pEapPeerMethodOutput);
                            }
                        }
                    } else
...
static DWORD _AddEAPMessage(_Inout_ AL::TLS::CSessionData *pSessionData, _In_bytecount_(dwEAPAttributeSize) LPCBYTE pbEAPAttribute, _In_ DWORD dwEAPAttributeSize, _Inout_ AL::EAP::CPacket &pktSend, _Out_ EapPeerMethodOutput* pEapPeerMethodOutput)
{
    ...
    DWORD dwEAPAVPSize;
    if ((dwReturnCode = AL::TLS::AuthMakeDiameterAttribute(0x4F, pbEAPAttribute, dwEAPAttributeSize, &pbEAPAVP, &dwEAPAVPSize)) == NO_ERROR) {
        if (!pSessionData->m_TLSSession.m_aState.IsEmpty()) {
            //
            // Append state attribute to EAP message.
            //
            LPBYTE pbStateAVP;
            DWORD dwStateAVPSize;
            if ((dwReturnCode = AL::TLS::AuthMakeDiameterAttribute(0x18, pSessionData->m_TLSSession.m_aState.GetData(), (DWORD)pSessionData->m_TLSSession.m_aState.GetCount(), &pbStateAVP, &dwStateAVPSize)) == NO_ERROR) {
                DWORD dwAVPSize = dwEAPAVPSize + dwStateAVPSize;
                LPBYTE pbAVP;
                if ((dwReturnCode = AL::Heap::Alloc(dwAVPSize, (LPVOID*)&pbAVP)) == NO_ERROR) {
                    memcpy(pbAVP,                pbEAPAVP,   dwEAPAVPSize);
                    memcpy(pbAVP + dwEAPAVPSize, pbStateAVP, dwStateAVPSize);

                    dwReturnCode = AL::TLS::AddEAPMessage(pSessionData, pbAVP, dwAVPSize, pktSend, pEapPeerMethodOutput);
                    AL::Heap::Free((LPVOID*)&pbAVP);
                }

                AL::Heap::Free((LPVOID*)&pbStateAVP);
            }
        } else {
            //
