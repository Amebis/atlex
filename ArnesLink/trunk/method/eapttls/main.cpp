...
DWORD		VERSION = 1;
BYTE		EAPTYPE = 21;
PWCHAR		EAPID = L"EAP-TTLS";
PWCHAR		EAPFRIENDLYID = L"SecureW2 EAP-TTLS";
PWCHAR		EAPDLLNAME = L"sw2_ttls.dll";
#ifndef _WIN32_WCE
...
/*
		if (wcslen(g_pwcCSDVersion) > 1) // SP1 or higher
			dwReturnCode = SW2_RegisterEapHostExtendedDLL();
		else
*/
...
/*
		if (g_dwMinorVersion > 1)
			dwReturnCode = SW2_UnregisterEapHostExtendedDLL();
		else
*/
