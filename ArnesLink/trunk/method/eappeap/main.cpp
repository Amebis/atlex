...
PWCHAR		EAPFRIENDLYID = L"SecureW2 PEAP";
PWCHAR		EAPDLLNAME = L"sw2_peap.dll";
#ifndef _WIN32_WCE
DWORD		EAPPROPERTIES = eapPropCipherSuiteNegotiation |
							eapPropMutualAuth |
							eapPropIntegrity |
							...
							0x10000000;
#else
DWORD		EAPPROPERTIES = 0x280000;
#endif // _WIN32_WCE
DWORD		EAPUSERNAMEDLG = 0;
DWORD		EAPPWDDLG = 0;
DWORD		EAPMPPESUPPORTED = 1;
...
DWORD SW2CopyRegKey(IN HKEY hKeyDst, IN HKEY hKeySrc)
{
	DWORD dwReturnCode;
...
DllRegisterServer( VOID )
{
	...
	if (g_dwMajorVersion > 5)
	{
		// VISTA
/*
		if (wcslen(g_pwcCSDVersion) > 1) // SP1 or higher
			return SW2_RegisterEapHostExtendedDLL();
		else
*/
			dwReturnCode = SW2_RegisterEapHostDLL();
	}
	else
	{
		dwReturnCode = SW2BackupEapMethod(EAPTYPE);
...
DllUnregisterServer( VOID )
{
	...
	if (g_dwMajorVersion > 5)
/*
		if (g_dwMinorVersion > 1)
			return SW2_UnregisterEapHostExtendedDLL();
		else
*/
		dwReturnCode = SW2_UnregisterEapHostDLL();
	else
	{
		if ((dwReturnCode = SW2_UnregisterRASEAPDLL()) == NO_ERROR)
		{
			dwReturnCode = SW2RestoreEapMethod(EAPTYPE);
		}
	}
